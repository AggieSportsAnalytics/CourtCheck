    ...
    <div className="mb-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {items.map((item, i) => (
          <div
            key={i}
            className="bg-secondary rounded-xl p-4 border border-gray-700/40 flex gap-3 items-start"
          >
            <div className={`mt-0.5 ${item.accent ? "text-accent" : has ? "text-gray-400" : "text-gray-600"}`}>
              {item.icon}
            </div>
            <div className="min-w-0">
              <p className="text-xs text-gray-400 truncate">{item.label}</p>
              <p className={`text-xl font-bold leading-tight ${has ? "text-white" : "text-gray-600"}`}>
                {item.value}
              </p>
      ...
      {/* Show notice when user has recordings but no tennis stats yet */}
      {!loading && !summary?.hasTennisStats && (
        <div className="mt-3 flex items-center gap-2.5 bg-yellow-500/8 border border-yellow-500/20 rounded-xl px-4 py-2.5 text-xs text-yellow-400/80">
          ...
          <span>
            ...
            <Link href="/upload" className="text-yellow-400 underline underline-offset-2 hover:text-yellow-300">
              Re-upload a match
            </Link>{" "}
            to generate them.
          </span>
