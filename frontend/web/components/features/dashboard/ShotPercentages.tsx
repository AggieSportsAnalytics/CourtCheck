...
type ShotType = 'forehand' | 'backhand' | 'slices' | 'lob' | 'drop';

const ShotPercentages = () => {
  const [activeSet, setActiveSet] = useState<'set1' | 'set2' | 'set3' | 'overall'>('overall');
  const chartRef = useRef<HTMLDivElement>(null);

  // Mock data for shot percentages
  const shotData: Record<ShotType, number> = {
    forehand: 22,
    backhand: 28,
    slices: 9,
    lob: 16,
    ...
    <div className="bg-white rounded-xl p-4">
      ...
      <div className="grid grid-cols-4 gap-1 mb-4">
        ...
        <button
          ...
          aria-label="View overall shot percentages"
        >
          Overall
        </button>
      </div>
      ...
      <div className="flex items-center justify-center mt-4">
        <div className="w-36 h-36 rounded-full border-4 border-gray-700 relative" ref={chartRef} aria-label="Shot type distribution pie chart">
          {/* This is a simple CSS pie chart - in a real app you'd use a chart library */}
          <div className="absolute inset-0 bg-[#a4de02] rounded-full" style={{ clipPath: 'polygon(50% 50%, 50% 0, 100% 0, 100% 22%, 50% 50%)' }} aria-hidden="true"></div>
          <div className="absolute inset-0 bg-[#436b22] rounded-full" style={{ clipPath: 'polygon(50% 50%, 100% 22%, 100% 50%, 78% 100%, 50% 50%)' }} aria-hidden="true"></div>
          <div className="absolute inset-0 bg-[#acc260] rounded-full" style={{ clipPath: 'polygon(50% 50%, 78% 100%, 50% 100%, 22% 100%, 50% 50%)' }} aria-hidden="true"></div>
          <div className="absolute inset-0 bg-[#225a28] rounded-full" style={{ clipPath: 'polygon(50% 50%, 22% 100%, 0 78%, 0 50%, 50% 50%)' }} aria-hidden="true"></div>
          <div className="absolute inset-0 bg-[#d4eb8f] rounded-full" style={{ clipPath: 'polygon(50% 50%, 0 50%, 0 22%, 0 0, 50% 0, 50% 50%)' }} aria-hidden="true"></div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-6">
        {(Object.entries(shotData) as [ShotType, number][]).map(([type, percentage]) => (
