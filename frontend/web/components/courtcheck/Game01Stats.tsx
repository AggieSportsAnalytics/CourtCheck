...
const Game01Stats = () => {
  ...
  const [activeTab2, setActiveTab2] = useState<'shots' | 'player'>('player');
  const [activeSet2, setActiveSet2] = useState<'set1' | 'set2' | 'set3' | 'overall'>('set1');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {['Set 1', 'Set 2', 'Set 3', 'Overall'].map((set, index) => (
                  <button
                    ...
                    onClick={() => setActiveSet(`set${index + 1}` as 'set1' | 'set2' | 'set3' | 'overall')}
                    aria-label={`View ${set}`}
                  >
                    {set}
                  </button>
              ...
              <div className="bg-[#90C641] rounded-2xl p-3">
                <div className="w-full aspect-[1.6] bg-[#A4D902] rounded-xl relative">
                  ...
                  <div className="absolute inset-[8%] border-2 border-white">
                    ...
                    {/* Service boxes */}
                    <div className="absolute top-[30%] left-[25%] h-[40%] w-0.5 bg-white"></div>
                    <div className="absolute top-[30%] left-[75%] h-[40%] w-0.5 bg-white"></div>
                    <div className="absolute top-[70%] left-0 right-0 h-0.5 bg-white"></div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {['Set 1', 'Set 2', 'Set 3', 'Overall'].map((set, index) => (
                  <button
                    ...
                    onClick={() => setActiveSet2(`set${index + 1}` as 'set1' | 'set2' | 'set3' | 'overall')}
                    aria-label={`View ${set}`}
                  >
                    {set}
                  </button>
        ...
        <div className="grid gap-4">
          ...
          <div className="bg-white rounded-xl p-4">
            ...
            {/* Bar Chart */}
            <div className="h-56 bg-gray-50 rounded-lg mt-4 relative" aria-label="Player movement bar chart">
            ...
            <div className="grid grid-cols-4 gap-1 mb-4">
              {['Set 1', 'Set 2', 'Set 3', 'Overall'].map((set) => (
                <button
                  ...
                  aria-label={`View ${set} shot percentages`}
                >
                  {set}
                </button>
              ))}
