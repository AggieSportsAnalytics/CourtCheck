...
class ActionRecognition:
    ...
    def add_frame(self, frame, player_box):
        ...
        box_center = (int((player_box[0] + player_box[2])/2), int((player_box[1] + player_box[3])/2))
                ...
                if ENABLE_STROKE_RECOGNITION and stroke_recognizer is not None:
                    for bbox, center, track_id in tracked_players_current_frame:
                        try:
                            ...
                            probs, stroke_label = stroke_recognizer.predict_stroke(frame, bbox)
                            # Store stroke info with player data
                            if track_id not in player_tracker_instance.track_history:
                                player_tracker_instance.track_history[track_id] = deque(maxlen=60)
                            player_tracker_instance.track_history[track_id].append((frame_count, center, stroke_label))
                        except Exception as e:
                            print(f"Error in stroke recognition for track_id {track_id}: {e}")

                # Process pose detection if enabled
                if ENABLE_POSE_DETECTION and pose_extractor is not None:
                    try:
                        # Extract player bounding boxes
                        player_boxes = [bbox for bbox, _, _ in tracked_players_current_frame]
                        if player_boxes:
                            # Extract pose and get stickman overlay
                            stickman_frame = pose_extractor.extract_pose(frame, player_boxes)
                            ...
                            if ENABLE_DRAWING and out_main is not None:
                                ...
                                output_frame = cv2.addWeighted(output_frame, 0.7, stickman_frame, 0.3, 0)
                    except Exception as e:
        ...
        if ENABLE_DRAWING and out_main is not None and out_minimap is not None:
            ...
            for bbox, center_pt, display_name in draw_persons_top_on_main_frame:
                if bbox is not None and len(bbox) == 4:
                    ...
                    if ENABLE_STROKE_RECOGNITION:
                        track_id = None
                        for bbox_t, center_t, track_id_t in tracked_players_current_frame:
                            if bbox_t == bbox:  # Match the bounding box to get the correct track_id
                                track_id = track_id_t
                                break
                        if track_id and track_id in player_tracker_instance.track_history:
                            history = player_tracker_instance.track_history[track_id]
                            if history:
                                last_entry = history[-1]
                                if len(last_entry) >= 3:  # Check if we have enough values
                    ...
                    if display_name is not None:
                        cv2.putText(
                            output_frame,
                            display_name + stroke_label,  # Add stroke label to display name
                            (x1, y1 - 5),
