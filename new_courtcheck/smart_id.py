...
import torch
import torch.hub  # Added for PyTorch Hub
...
# Imports for torchreid
import torchreid


# Re-ID Model Class (OSNet)
class ReIDModel:
    def __init__(self, model_name="osnet_x1_0_market1501", device="cuda"):
        self.device = device
        self.model = None
        try:
            # Infer base model name and dataset from the provided model_name
            architecture = model_name  # Default to full name if not clearly separable
            dataset_name = None  # Will be inferred by torchreid if possible

            # Try to parse standard format like 'osnet_x1_0_market1501'
            if "_market1501" in model_name:
                ...
                dataset_name = "msmt17"
            # Add other common dataset suffixes if needed

            print(
                f"[ReIDModel] Attempting to build model: '{architecture}' (intended dataset: '{dataset_name if dataset_name else 'any compatible'}')"
            )

            self.model = torchreid.models.build_model(
                name=architecture,
                num_classes=1000,  # Placeholder, often ignored if pretrained=True and stripping classifier
                loss="softmax",  # Placeholder
                pretrained=True,  # Key: try to load torchreid/TorchHub standard pretrained weights
            )

            # If pretrained=True doesn't load for the specific dataset,
            # and you have a direct .pth file, you might use:
            # weight_path = "/path/to/your/osnet_x1_0_market1501_weights.pth"
            # torchreid.utils.load_pretrained_weights(self.model, weight_path)
            # For now, we rely on pretrained=True to find standard weights.

            self.model.to(self.device)
            self.model.eval()
            print(
                f"[ReIDModel] Successfully built model '{architecture}' with its standard pretrained weights."
            )

        except Exception as e:
            print(f"[ReIDModel] Error loading model '{model_name}' with torchreid: {e}")
            print(
                "[ReIDModel] Ensure 'torchreid' is installed and the model name is a standard one recognized by torchreid."
            )
            print(
                "[ReIDModel] Falling back to placeholder; dummy features will be used."
            )
            self.model = None  # Explicitly ensure model is None on any exception
    ...
    def extract_features(self, image_patch_numpy):
        if self.model is None:  # This check is vital
            # print("[ReIDModel] Model not loaded. Returning dummy features.")
            return torch.randn(512).numpy()

        # If self.model is not None, proceed with feature extraction
        try:
            self.model.to(self.device)
            # ... (rest of the BGR to RGB, transform, and inference logic remains the same)
            if image_patch_numpy.shape[2] == 3:
                img_rgb = cv2.cvtColor(image_patch_numpy, cv2.COLOR_BGR2RGB)
            else:
                img_pil = T.ToPILImage()(image_patch_numpy)
            ...
            )  # Assuming features are [1, N], squeeze to [N]
        except Exception as e:
            print(f"[ReIDModel] Error extracting features: {e}")
            # Fallback to dummy features if something goes wrong during extraction
            return torch.randn(512).numpy()
...
class PlayerTracker:
    def __init__(
        ...
        device="cuda",
        reid_model_path="osnet_x1_0_market1501.pt",
        ...
        self.reid_model = ReIDModel(model_name=reid_model_path, device=device)
        self.reid_similarity_threshold = reid_similarity_threshold

        self.court_ref = CourtReference()
        self.assigned_player_roles = (
            {}
        )  # Stores {track_id: {"role": "Player 1", "features": np.array}}

        # New attributes for initial features
        self.initial_player1_features = None
        self.initial_player2_features = None

        self.generic_player_labels = {}
        self.player_role_counter = 1
        self.next_generic_id_counter = 1

        self.lost_player_roles = {}
        self.MAX_MISSED_FRAMES_FOR_REID = 60
    ...
    def update(self, frame, person_min_score=0.5):
        # Output of this function will now be List[(bbox, center, track_id)]
        # The display name generation is moved to get_player_display_name
        current_detections_for_output = []
        ...
        current_raw_detections = []  # temp list of (bbox, center, tid) from yolo
        if results and results[0].boxes is not None and results[0].boxes.id is not None:
            ...
            for i, tid in enumerate(track_ids):
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    # Add to output list directly, display name will be resolved later
                    current_detections_for_output.append(
                        (bbox_for_output, (bottom_center_x, bottom_center_y), tid)
                    )

        # --- Manage Lost Player Roles (P1/P2 specifically) ---
        # Increment missed frames counts
        for role_name in list(self.lost_player_roles.keys()):
            self.lost_player_roles[role_name]["missed_frames"] += 1
            if (
                self.lost_player_roles[role_name]["missed_frames"]
                > self.MAX_MISSED_FRAMES_FOR_REID
            ):
                print(
                    f"[PlayerTracker Update] Role {role_name} (last_track_id: {self.lost_player_roles[role_name]['last_track_id']}) truly lost after {self.MAX_MISSED_FRAMES_FOR_REID} frames."
                )
                # Consider if features should be cleared or kept for a longer-term re-id strategy not tied to this counter
                del self.lost_player_roles[role_name]

        # Identify which of the initially assigned P1/P2 are missing in the current frame
        # This logic is primarily for updating self.lost_player_roles
        for role_to_check, initial_features in [
            ("Player 1", self.initial_player1_features),
            ("Player 2", self.initial_player2_features),
        ]:
            if initial_features is None:  # This player was never initialized
                continue

            # Find if any current track_id is currently considered this role
            # This check is tricky because roles can be reassigned by get_player_display_name
            # For simplicity here, we check against self.assigned_player_roles which holds the *last known stable* role for a track_id.

            is_role_active = False
            current_tid_for_this_role = None
            for tid, role_data in self.assigned_player_roles.items():
                if (
                    role_data["role"] == role_to_check
                    and tid in current_frame_track_ids
                ):
                    is_role_active = True
                    current_tid_for_this_role = tid
                    break

            if is_role_active:
                # If the role is active (seen in current frame by a track_id associated with it)
                # and it was in lost_player_roles, remove it from lost.
                if role_to_check in self.lost_player_roles:
                    print(
                        f"[PlayerTracker Update] Role {role_to_check} (now track_id {current_tid_for_this_role}) re-found. Clearing from lost_player_roles."
                    )
                    del self.lost_player_roles[role_to_check]
            else:
                # Role is not active in the current frame
                if role_to_check not in self.lost_player_roles:
                    # Find the last track_id that held this role to store its features
                    last_tid_for_role = None
                    last_features_for_role = (
                        initial_features  # Default to initial features
                    )

                    # Find the most recent track_id associated with this role from self.assigned_player_roles
                    # This is to get the most up-to-date features if they were being updated (though not explicitly implemented yet)
                    # For now, this part is a bit redundant if we always use initial_features for P1/P2 matching.
                    # However, good to have a placeholder for last_track_id.
                    for tid_iter, role_data_iter in self.assigned_player_roles.items():
                        if role_data_iter["role"] == role_to_check:
                            last_tid_for_role = tid_iter
                            # If self.assigned_player_roles[tid_iter]["features"] were being updated, use that.
                            # For now, we assume P1/P2 features in self.assigned_player_roles match initial_features.
                            # So, 'last_features_for_role' remains 'initial_features'.
                            break  # Found the last known track id for this role

                    print(
                        f"[PlayerTracker Update] Role {role_to_check} (last known track_id: {last_tid_for_role if last_tid_for_role else 'None'}) lost. Storing initial features for Re-ID."
                    )
                    self.lost_player_roles[role_to_check] = {
                        "last_track_id": last_tid_for_role,  # Can be None if role was just initialized and lost immediately
                        "features": initial_features,  # Always use initial features for P1/P2 as per request
                        "missed_frames": 0,
                    }

        # The complex re-identification logic that tries to match new tracks to self.lost_player_roles
        # for P1/P2 assignment is removed from here. That primary P1/P2 assignment will happen in get_player_display_name
        # based on initial features. This 'update' method can still handle re-id for generic tracks if needed in future.

        return current_detections_for_output  # List of (bbox, center, track_id)

    def assign_initial_player_roles(self, frame, tracked_players_on_first_frame):
        if not tracked_players_on_first_frame:
            print("[PlayerTracker] No tracked players on first frame to assign roles.")
            return

        unique_tracks_for_assignment = []  # (track_id, bbox)
        seen_ids = set()
        for bbox, center, track_id in tracked_players_on_first_frame:
            if track_id not in seen_ids:
                unique_tracks_for_assignment.append({"id": track_id, "bbox": bbox})
                seen_ids.add(track_id)

        # Sort by horizontal position (x-coordinate of bbox center) to try and get left/right consistently
        # This is a heuristic and might need adjustment based on typical camera angles.
        # For simplicity, using x1 of bbox.
        unique_tracks_for_assignment.sort(key=lambda x: x["bbox"][0])

        for track_info in unique_tracks_for_assignment:
            track_id = track_info["id"]
            bbox = track_info["bbox"]

            if self.player_role_counter <= 2:
                # Check if this track_id already has a P1/P2 role (e.g. if this function is called multiple times near frame 0)
                # This check is a bit complex due to dict structure. Simpler: just check if P1/P2 features are already set.

                role_to_assign = None
                if (
                    self.player_role_counter == 1
                    and self.initial_player1_features is None
                ):
                    role_to_assign = "Player 1"
                elif (
                    self.player_role_counter == 2
                    and self.initial_player2_features is None
                ):
                    role_to_assign = "Player 2"

                if role_to_assign:
                    player_patch = self._get_player_patch(frame, bbox)
                    features = None
                    if player_patch.size > 0:
                        features = self.reid_model.extract_features(player_patch)

                    if features is not None:
                        if role_to_assign == "Player 1":
                            self.initial_player1_features = features
                        elif role_to_assign == "Player 2":
                            self.initial_player2_features = features

                        # Store in assigned_player_roles as well for consistency and potential fallback
                        self.assigned_player_roles[track_id] = {
                            "role": role_to_assign,
                            "features": features,  # Storing initial features here too
                        }
                        print(
                            f"[PlayerTracker] Assigned role '{role_to_assign}' to track_id {track_id} and stored its initial features."
                        )
                        self.player_role_counter += 1
                    else:
                        print(
                            f"[PlayerTracker] Could not extract features for track_id {track_id} for role {role_to_assign}. Role not assigned."
                        )
            else:
                break  # Already assigned P1 and P2

        if self.initial_player1_features is not None:
            print("[PlayerTracker] Initial Player 1 features stored.")
        if self.initial_player2_features is not None:
            print("[PlayerTracker] Initial Player 2 features stored.")

    def get_player_display_name(
        self, frame, bbox_current_detection, track_id_current_detection
    ):
        current_features = None
        player_patch = self._get_player_patch(frame, bbox_current_detection)
        if player_patch.size > 0:
            current_features = self.reid_model.extract_features(player_patch)

        matched_role_from_initial = None
        best_similarity_to_initial = -1

        if current_features is not None:
            sim_p1 = -1
            if self.initial_player1_features is not None:
                try:
                    sim_p1 = F.cosine_similarity(
                        torch.from_numpy(current_features).unsqueeze(0),
                        torch.from_numpy(self.initial_player1_features).unsqueeze(0),
                    ).item()
                except Exception as e_sim1:
                    print(f"[WARN] Cosine similarity error for P1: {e_sim1}")

            sim_p2 = -1
            if self.initial_player2_features is not None:
                try:
                    sim_p2 = F.cosine_similarity(
                        torch.from_numpy(current_features).unsqueeze(0),
                        torch.from_numpy(self.initial_player2_features).unsqueeze(0),
                    ).item()
                except Exception as e_sim2:
                    print(f"[WARN] Cosine similarity error for P2: {e_sim2}")

            # Determine match based on similarity scores
            p1_matches = sim_p1 > self.reid_similarity_threshold
            p2_matches = sim_p2 > self.reid_similarity_threshold

            if p1_matches and p2_matches:
                if (
                    sim_p1 >= sim_p2
                ):  # Prefer P1 in case of a tie or higher P1 similarity
                    matched_role_from_initial = "Player 1"
                    best_similarity_to_initial = sim_p1
                else:
                    matched_role_from_initial = "Player 2"
                    best_similarity_to_initial = sim_p2
            elif p1_matches:
                matched_role_from_initial = "Player 1"
                best_similarity_to_initial = sim_p1
            elif p2_matches:
        ...
        if matched_role_from_initial:
            # If matched by initial appearance, this is the primary role.
            # Update assigned_player_roles to reflect this current assignment for this track_id
            # This helps if the same track_id (from BoT-SORT) gets re-identified as P1/P2
            if track_id_current_detection in self.assigned_player_roles:
                self.assigned_player_roles[track_id_current_detection][
                    "role"
                ] = matched_role_from_initial
                # Optionally update features if this is considered a strong re-affirmation
                # self.assigned_player_roles[track_id_current_detection]["features"] = current_features
            else:
                self.assigned_player_roles[track_id_current_detection] = {
                    "role": matched_role_from_initial,
                    "features": (
                        current_features
                        if current_features is not None
                        else np.array([])
            ...
            # print(f"Track {track_id_current_detection} matched to initial {matched_role_from_initial} with sim {best_similarity_to_initial:.2f}")
            return matched_role_from_initial

        # Fallback: If not matched to initial P1/P2 by current appearance
        # Check if this track_id was previously known (either as P1/P2 or generic)
        if track_id_current_detection in self.assigned_player_roles:
            # This track_id was involved in initial role assignment or a later re-id by the old 'update' logic.
            # Return its last known established role.
            return self.assigned_player_roles[track_id_current_detection]["role"]

        if track_id_current_detection in self.generic_player_labels:
            return self.generic_player_labels[track_id_current_detection]

        # Assign a new generic label if completely new and not matched
        label = f"TrackID-{self.next_generic_id_counter}"
        self.generic_player_labels[track_id_current_detection] = label
        self.next_generic_id_counter += 1
        # print(f"Track {track_id_current_detection} assigned generic label {label}")
        return label
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if process_this_frame:
            ...
            # Only run player tracking if homography was successful for this frame (or carried over)
            # And only update player_tracker_instance if it's a processing frame.
            if (
                current_homography is not None
            ):  # Ensure homography is valid before person detection
                ...
                if (
                    frame_count == 0
                ):  # Or first frame where tracked_players_current_frame is not empty
                    # Pass the frame itself for patch extraction
                    player_tracker_instance.assign_initial_player_roles(
                        frame, tracked_players_current_frame
                    )

            last_processed_homography = current_homography
            last_processed_kps = current_kps
            # last_processed_persons_top and _bottom are implicitly handled by how data is stored and retrieved
        else:
            # Carry over homography and kps from the last processed frame
            current_homography = last_processed_homography
            current_kps = last_processed_kps
            # Run player tracking on every frame even if not a detector processing frame,
            # using the latest available homography for court context if needed by tracker internals (though current one doesn't directly use it in update)
            tracked_players_current_frame = player_tracker_instance.update(
                frame
            )  # Pass the whole frame
        ...
        if current_homography is not None and tracked_players_current_frame:
            for bbox, center_on_frame, track_id in tracked_players_current_frame:
                display_name = player_tracker_instance.get_player_display_name(
                    frame, bbox, track_id
                )
